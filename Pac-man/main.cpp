...
int main(int argv, char** args)
{
    ...
    if( init(&window, &renderer) )
    {
        ...
        stage_path.push_back("sprites/super_wall.png");
            ...
            and loadSounds(sounds, tracks, sound_path, track_path))
        {
            ...
            bool quit = false;
            if(classic_level(layout, renderer, normal_stage_textures, font, sounds, quit))
            {
                bool victory;
                display_splash(renderer,splash_screens[0], 3000, quit);
                display_splash(renderer,splash_screens[1], 3000, quit);
                boss_fight(renderer, boss_stage_textures, font, victory, quit);
                if (victory)
                    display_splash(renderer, splash_screens[0], 3000, quit);
                else
                    display_splash(renderer, splash_screens[2], 3000, quit);
            }
            else
                display_splash(renderer, splash_screens[2], 3000, quit);
